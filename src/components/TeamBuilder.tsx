...
export default function TeamBuilder({ team, onRemove, onSelect }: Props) {
  ...
  const getTeamTypes = () => {
    ...
    team.forEach((p) => p.types.forEach((t: any) => {
  ...
  const totalStats = () => {
    ...
    team.forEach((p) => p.stats.forEach((s: any) => {
  ...
  return (
    <div className="team-builder">
      <div className="team-header">
        <h2>MON ÉQUIPE <span>{team.length}/6</span></h2>
      ...
      <div className="team-slots">
        {slots.map((pokemon, i) => (
          <div
            ...
            style={pokemon ? { '--type-color': TYPE_COLORS[pokemon.types[0].type.name] || '#ccc' } as React.CSSProperties : {}}
          >
            {pokemon ? (
              <>
                ...
                <div className="slot-info">
                  <span className="slot-num">#{String(pokemon.id).padStart(3,'0')}</span>
                ...
                <button className="slot-remove" onClick={() => onRemove(pokemon.id)}>✕</button>
              </>
            ) : (
            ...
            <div className="team-types-wrap">
              {Object.entries(teamTypes).map(([type, count]) => (
                <div key={type} className="team-type-row">
                  <span className="type-tag" style={{ background: TYPE_COLORS[type] || '#ccc' }}>{type}</span>
                  <div className="type-count-bar">
                    <div className="type-count-fill" style={{ width: `${count * 33}%`, background: TYPE_COLORS[type] }} />
                  </div>
      ...
      {team.length === 0 && (
        <div className="team-empty-state">
          ...
          <span>Va dans le Pokédex et clique sur <strong>+</strong> pour ajouter des Pokémon.</span>
        </div>
      )}
    </div>
  )
