...
export default function PokemonDetail({
  ...
  const displayId = pokemon.displayId || String(pokemon.id).padStart(3, '0')
  const sprite =
  ...
  return (
    <div className="detail-card" style={{ '--type-color': color } as React.CSSProperties}>
      ...
      <div className="detail-header">
        <div className="detail-id">#{displayId}</div>
          ...
          <div className="moves-list">
            {pokemon.moves && pokemon.moves.length > 0 ? (
              pokemon.moves.map((m: any) => (
                <span key={m.move.name} className="move-tag">
                  {m.move.name.replace(/-/g, ' ')}
                </span>
              ))
            ) : (
              <p className="no-moves">Aucune capacité renseignée</p>
            )}
          </div>
        )}
